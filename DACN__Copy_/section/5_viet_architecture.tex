...
\indent Hệ thống được thiết kế theo mô hình kiến trúc phân tầng (Layered Architecture) điển hình của các hệ thống IoT công nghiệp (IIoT). Kiến trúc được chia thành 3 tầng chính: Tầng Vật lý (Physical Layer), Tầng Biên (Edge Layer), và Tầng Dữ liệu $\&$ Ứng dụng (Cloud/Application Layer).
\indent Mô hình này đảm bảo sự tách biệt rõ ràng giữa nhiệm vụ thu thập phần cứng (CE contribution) và xử lý dữ liệu nâng cao (CS contribution).

\subsubsection{Sơ đồ kiến trúc tổng thể}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{img/Firmware/Architecture overall mermaid img.png}
    \caption{Sơ đồ Kiến trúc Tổng thể của Hệ thống}
    \label{fig:architecture_overall}
\end{figure}

\subsubsection{Chi tiết các tầng}
\subsubsubsection{Tầng Vật lý (Physical Layer)}
\indent Tầng này bao gồm các thiết bị đầu cuối chịu trách nhiệm tương tác trực tiếp với môi trường:
\begin{itemize}
    \item \textbf{Cụm cảm biến môi trường:} Sử dụng các cảm biến công nghiệp (SHTC3, pH, EC) giao tiếp qua chuẩn Modbus RTU (RS485). Việc sử dụng chuẩn này giúp tín hiệu truyền đi xa (lên đến 1200m) và kháng nhiễu tốt trong môi trường khắc nghiệt.
    \item \textbf{Thiết bị quan sát:} Camera IP Hikvision cung cấp dữ liệu hình ảnh thời gian thực về khu vực giám sát.
\end{itemize}
\subsubsubsection{Tầng Biên (Edge Layer)}
\indent Đây là tầng trung gian xử lý dữ liệu tại nguồn, được chia thành hai node chức năng chuyên biệt để tối ưu hóa tài nguyên phần cứng:
\begin{itemize}
    \item \textbf{Telemetry Node (Sử dụng ESP32):}
    \begin{itemize}
        \item Đóng vai trò là Modbus Master, truy vấn dữ liệu từ các cảm biến theo chu kỳ.
        \item Kiểm tra toàn vẹn dữ liệu: Thực hiện tính toán mã lỗi CRC-16 để loại bỏ các gói tin bị nhiễu trên đường truyền RS485.
        \item Gán nhãn chất lượng (Quality Flagging): Thực hiện so sánh sơ cấp để gán nhãn trạng thái cho dữ liệu (ví dụ: Flag=0 là tốt, Flag=1 là nghi ngờ lỗi gai/trôi) trước khi gửi đi.
    \end{itemize}
    \item \textbf{Vision Gateway (Sử dụng Raspberry Pi):}
    \begin{itemize}
        \item Tiếp nhận luồng video (RTSP Stream) từ Camera Hikvision.
        \item Đồng bộ thời gian (Time Synchronization): Sử dụng giao thức NTP để đảm bảo dữ liệu hình ảnh và dữ liệu cảm biến được đồng bộ chính xác về mặt thời gian (Timestamp alignment), phục vụ cho việc đối soát dữ liệu sau này.
    \end{itemize}
\end{itemize}
\subsubsubsection{Tầng Dữ liệu \& Ứng dụng (Cloud/Application Layer)}
\indent Tầng này chịu trách nhiệm lưu trữ lâu dài và xử lý dữ liệu chuyên sâu:
\begin{itemize}
    \item \textbf{Cơ sở dữ liệu TimescaleDB:} Là một hệ quản trị cơ sở dữ liệu chuỗi thời gian (Time-series Database) được tối ưu hóa để lưu trữ lượng lớn dữ liệu cảm biến với tốc độ truy vấn cao.
    \item \textbf{Thuật toán RFE (Robust Feature Extraction):} Đây là thành phần xử lý thông minh cốt lõi. Thuật toán nhận đầu vào là dữ liệu thô (kèm Quality Flag từ ESP32), thực hiện các phép toán thống kê bền vững (Robust Statistics) để trích xuất các đặc trưng chính xác, loại bỏ hoàn toàn ảnh hưởng của nhiễu Drift hoặc Spike.
    \item \textbf{Dashboard:} Giao diện người dùng hiển thị trực quan các thông số môi trường đã được làm sạch và hình ảnh giám sát thời gian thực.
\end{itemize}
